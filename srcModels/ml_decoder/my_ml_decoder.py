...
class MLDecoder(nn.Module):
    def __init__(self, num_classes, num_of_groups=-1, decoder_embedding=768,
        ...
        if not zsl:
            query_embed = nn.Embedding(13, decoder_embedding)
            query_embed.requires_grad_(False)

            query_embed_l = nn.Embedding(16, decoder_embedding)
            query_embed_l.requires_grad_(False)
        ...
        self.decoderl = nn.TransformerDecoder(layer_decode, num_layers=num_layers_decoder)
        self.decoder.embed_standart = embed_standart
        self.decoderl.embed_standart = embed_standart
        self.decoder.query_embed = query_embed
        self.decoderl.query_embed = query_embed_l
        self.zsl = zsl
        ...
        if self.zsl:
            ...
            self.decoder.num_classes = 13
            self.decoder.duplicate_factor = int(13 / 13 + 0.999)
            self.decoder.duplicate_pooling = torch.nn.Parameter(
                torch.Tensor(13, decoder_embedding, self.decoder.duplicate_factor))
            self.decoder.duplicate_pooling_bias = torch.nn.Parameter(torch.Tensor(13))

            # group fully-connected
            self.decoderl.num_classes = 16
            self.decoderl.duplicate_factor = int(16 / 16 + 0.999)
            self.decoderl.duplicate_pooling = torch.nn.Parameter(
                torch.Tensor(16, decoder_embedding, self.decoder.duplicate_factor))
            self.decoderl.duplicate_pooling_bias = torch.nn.Parameter(torch.Tensor(16))

        torch.nn.init.xavier_normal_(self.decoder.duplicate_pooling)
        torch.nn.init.constant_(self.decoder.duplicate_pooling_bias, 0)
        torch.nn.init.xavier_normal_(self.decoderl.duplicate_pooling)
        torch.nn.init.constant_(self.decoderl.duplicate_pooling_bias, 0)
        self.decoder.group_fc = GroupFC(13)
        self.decoderl.group_fc = GroupFC(16)
    ...
    def forward(self, x):
        ...
        if self.zsl:
            ...
            query_embedl = self.decoderl.query_embed.weight
        # tgt = query_embed.unsqueeze(1).repeat(1, bs, 1)
        tgt = query_embed.unsqueeze(1).expand(-1, bs, -1)  # no allocation of memory with expand
        h = self.decoder(tgt, embedding_spatial_786.transpose(0, 1))  # [embed_len_decoder, batch, 768]
        h = h.transpose(0, 1)

        tgt_l = query_embedl.unsqueeze(1).expand(-1, bs, -1)  # no allocation of memory with expand
        h_l = self.decoderl(tgt_l, embedding_spatial_786.transpose(0, 1))  # [embed_len_decoder, batch, 768]
        ...
        out_extrap_l = torch.zeros(h_l.shape[0], h_l.shape[1], self.decoderl.duplicate_factor, device=h_l.device, dtype=h.dtype)

        self.decoder.group_fc(h, self.decoder.duplicate_pooling, out_extrap)
        self.decoder.group_fc(h_l, self.decoderl.duplicate_pooling, out_extrap_l)

        if not self.zsl:
            h_out = out_extrap.flatten(1)[:, :self.decoder.num_classes]
            h_out_l = out_extrap_l.flatten(1)[:, :self.decoderl.num_classes]
        else:
            h_out = out_extrap.flatten(1)
        h_out += self.decoder.duplicate_pooling_bias
        h_out_l += self.decoderl.duplicate_pooling_bias
        logits = h_out
        logits_l = h_out_l
        return logits, logits_l
